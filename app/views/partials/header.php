...
$title = isset($title) ? $title : 'Student Information';
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<div class="max-w-6xl mx-auto px-4">
		<div class="flex items-center justify-between py-3 px-4 rounded-xl bg-white/3 backdrop-blur-md shadow-sm">
			<div class="flex items-center gap-3">
				<div class="w-10 h-10 flex items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-cyan-400 text-white font-extrabold text-lg shadow-inner">SI</div>
				<div class="text-sm text-white/90">Student Information</div>
			</div>
			<div class="flex items-center gap-3">
				<?php if (function_exists('lava_instance') && lava_instance()->session->userdata('user_id')): ?>
					...
					?>
					<div class="flex items-center gap-3">
						<?php if (function_exists('avatar_for')): ?>
							<?= call_user_func('avatar_for', $me['email'] ?? null, $displayName, 40) ?>
						<?php else: ?>
							<div class="w-10 h-10 inline-flex items-center justify-center rounded-full bg-indigo-500 text-white font-extrabold">SI</div>
						<?php endif; ?>
